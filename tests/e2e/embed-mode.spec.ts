...
const dashboardPayload = {
  profile: {
    ...
    biography: "Perfil de teste com dados simulados para o modo embed.",
...
test("keeps the embed presentation and query flag through the profile journey", async ({ page }) => {
  await mockTrpc(page);
  await page.goto("/?embed=1");

  await expect(page.locator("body")).toHaveClass(/embed-mode/);
  await expect(page.getByLabel("Voltar para a busca")).toHaveCount(0);
  ...
  await expect(page).toHaveURL(/\/perfil\/demoprofile\?range=30d&sort=best&embed=1/);
  await expect(page.locator("body")).toHaveClass(/embed-mode/);
  await expect(page.getByRole("heading", { name: "Demo Profile" })).toBeVisible();
  await expect(page.getByLabel("Voltar para a busca")).toHaveCount(0);
});
